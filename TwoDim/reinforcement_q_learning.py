...
def get_screen():
    ...
    print("(1)shape={}, type={}".format(screen.shape, type(screen)))
    # Convert to float, rescare, convert to torch tensor
    # (this doesn't require a copy)
    screen = np.ascontiguousarray(screen, dtype=np.float32) / 255
    print("(2)shape={}, type={}".format(screen.shape, type(screen)))
    screen = torch.from_numpy(screen)
    print("(3)shape={}, type={}".format(screen.shape, type(screen)))
    # Resize, and add a batch dimension (BCHW)
    qqq = resize(screen).unsqueeze(0).to(device)
    print("(4)shape={}, type={}".format(qqq.shape, type(qqq)))
    return qqq
