...
def run_main(mdp, agent, num_episodes, max_episode_len):
    ...
    for num_episode in range(num_episodes):
        ...
        for i in range(max_episode_len):
            ...
            next_state_full = compact2full(next_state, mdp.shape)
            ...
            next_state_full_torch = torch.Tensor([next_state_full])
...
def run_dqn(random_seed=142, shape=(5,6), **kwargs):
    ...
    # The Agent
    dqn_parameters = {"dim_state": np.prod(shape),
...
if __name__ == "__main__":
    run_dqn(num_episodes=100, max_episode_len=200)
